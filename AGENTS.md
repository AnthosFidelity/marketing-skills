...
| [`customer-research`](./skills/customer-research) | Mine Reddit, YouTube comments, G2/Capterra, X/Twitter, and TikTok to surface what customers actually say in their own words. Produces research synthesis reports, VOC quote banks, personas, and JTBD maps. No paid integrations required — uses built-in Hyper scrapers. Ships with two references under `references/` (per-source playbooks, synthesis templates). |
| [`youtube-transcript`](./skills/youtube-transcript) | Fetch the full transcript of any YouTube video (`youtube_transcript`) and optionally use AI-powered extraction (`youtube_reader`) to turn it into blog posts, social content, summaries, show notes, timestamped chapters, or raw quote banks. No integrations required. |
